...
class ImportSkpnPresenter extends BasePresenter {
    ...
    public function parseSkpnResultListFile(Form $form, $values) {
        ...
        if($matches == [[]]) { // pokud nenajdeme žádné čísla, je to chyba
            ...
            if($this->user->id == 5) 
            	$persons = $this->person->findBy(['center.title' => "CVS Lužce"])
                                ->order('name ASC');

            foreach ($persons as $person) {
                $persons_pairs[$person->id] = $person->name." (".$person->center->abbreviation.")";
            }

            $centers_pairs = $this->center->findAll()->order('title ASC')
                                                     ->fetchPairs('id', 'title');
            ...
            foreach ($score_data_lines as $line) {
                ...
                $container->addSelect('center_id', "", $centers_pairs)
                          ->setPrompt('--- vyber centrum ---');
                ...
                if(isset($person->id)) {
                    ...
                    $container['center_id']->setDefaultValue($person->skpn_center_id);
                }
        ...
        foreach ($persons as $person) {
	        if(isset($person['do_import']) && $person['do_import'] == "on") {
            	if($person['person_id'] != "") {
        			...
        			$this->distribution->insertResult($person['person_id'], $week, $year, $person['center_id'], $maha_book->id, $person['maha']);
        			$this->distribution->insertResult($person['person_id'], $week, $year, $person['center_id'], $big_book->id, $person['big']);
        			$this->distribution->insertResult($person['person_id'], $week, $year, $person['center_id'], $medium_book->id, $person['medium']);
        			$this->distribution->insertResult($person['person_id'], $week, $year, $person['center_id'], $small_book->id, $person['small']);
        			$this->distribution->insertResult($person['person_id'], $week, $year, $person['center_id'], $mag_book->id, $person['mag']);

                    $persons_alias = $this->person->findBy(['skpn_alias' => $person['skpn_alias']]);

                    foreach ($persons_alias as $person_alias) {
                        $this->person->update($person_alias->id, ['skpn_alias' => NULL, 'skpn_center_id' => NULL]);
                    }

                    $this->person->update($person['person_id'], ['skpn_alias' => $person['skpn_alias'], 'skpn_center_id' => $person['center_id']]);
            	}
            }  
        }
